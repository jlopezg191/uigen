...
function toolLabel(toolName: string, args: Record<string, unknown>): string {
  const filename = typeof args.path === "string" ? args.path.split("/").pop() : null;
  ...
  if (toolName === "file_manager") {
    switch (args.command) {
      ...
      case "rename": {
        const newName = typeof args.new_path === "string" ? args.new_path.split("/").pop() : null;
        return newName ? `Renaming ${name} to "${newName}"` : `Renaming ${name}`;
      }
      default: return `Managing ${name}`;
    }
  }
